  ...
  ,doRender: function(cs, rs, ds, startRow, colCount, stripe){
     ...
     for (var i = 0, len = rs.length; i < len; i++) {
       ...
       for (var j = 0; j < gfLen; j++) {
         ...
         fieldLabel = this.cm.lookup[colIndex].header;
         v = r.data[fieldName];
         if (v) {
           if (i == 0) {
             // First record always starts a new group
             addGroup.push({idx:j,dataIndex:fieldName,header:fieldLabel,value:v,colIndex:colIndex});
             lastvalues[j] = v;
           ...
           if (this.displayEmptyFields) {
             addGroup.push({idx:j,dataIndex:fieldName,header:fieldLabel,value:this.emptyGroupText||'(none)',colIndex:colIndex});
             grpFieldNames.push(fieldName);
...
//         if (!curGroup || curGroup.group != gp.dataIndex || currGroup.gvalue != gp.value) {
           ...
           gid = this.constructId(gp.value, gp.dataIndex, colIndex);
  ...
  ,getRowsFromGroup: function(r, gs, lsField){
    ...
    processEvent: function(name, e){
        ...
        if(hd){
            ...
            var prefixOnly = this.getPrefix(''),
            	field = hd.id.substr(prefixOnly.substring(0, prefixOnly.length - 1).length,hd.id.length);
